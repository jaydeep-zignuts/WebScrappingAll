const { getLinks, scrape, bike, bikeDetails, downloadImage } =require ('../helper/aoicycle.helper');
const rules  = require('../constants/aoicycle.constant');
const db = require('../models');
const bikedetailsModel = require('../models/bikedetails.model');
const aoiCycleDetails = db.aoiCycle;
...
exports.getBikeCardLink = async(req, res)=>{
    ...
    const result =await scrape().then(async (url)=>{
        ...
        for(let ind= 0; ind<url.length;ind++){ 
            await aoiCycleDetails.create({url : url[ind]["url"]}).then(r=>{
                console.log("created");
            }).catch(err=>{
...
exports.getBikeDetails= async(req, res) =>{
    const findData =  aoiCycleDetails.findAll({attributes:['url']}).then( result => {
        console.log(result)
...
exports.downloadImage =(req, res) =>{
    aoiCycleDetails.findAll({attributes:['image']}).then((result)=>{
