const { getLinks, scrape, bike, bikeDetails, downloadImage } =require ('../helper/bikefocus.helper');


const { constants, rules } = require('../constants/bikeDiamantrad.constant');
const db = require('../models');
const bikedetailsModel = require('../models/bikedetails.model');
const DiamantradBikeDetails = db.DiamantradBikeDetails;
...
exports.getBikeCardLink = async(req, res)=>{
    ...
    const result =await scrape().then(async (url)=>{
        ...
        for(let ind= 0; ind<url.length;ind++){ 
            await DiamantradBikeDetails.create({url : url[ind]["url"]}).then(r=>{
                console.log("created");
            }).catch(err=>{
...
exports.getBikeDetails= async(req, res) =>{
    const findData =  DiamantradBikeDetails.findAll({attributes:['url']}).then( result => {
        // console.log(result)
            ...
            // console.log("result of url ==> ",result);
             bikeDetails(result[i]['dataValues']['url']).then(data =>{
                ...
                DiamantradBikeDetails.update({
                    ...
                    frame: data[0]['frame'],
                    fork: data[0]['fork'],
                    color: data[0]['bike_color']?.replace(/\s\s+/g, ' '),
...
exports.downloadImage =(req, res) =>{
    DiamantradBikeDetails.findAll({attributes:['image']}).then((result)=>{
...
// https://media.trekbikes.com/image/upload/018HER_21_33221_B_Primary?$responsive-pjpg$&cache=on,on&wid=1920&hei=1440;
// https://media.trekbikes.com/image/upload/018HER_21_33221_A_Primary?$responsive-pjpg$&cache=on,on&wid=1920&hei=1440;
// https://media.trekbikes.com/image/upload/018_HCH_23_33221_A_Primary?$responsive-pjpg$&cache=on,on&wid=1920&hei=1440